...
import {
  SPIRITUAL_PLACES,
  getSpiritualPlaceBySlug,
} from "../place-data";

type SpiritualPlacePageProps = {
  params: {
    slug: string;
  };
};
...
export function generateStaticParams() {
  return SPIRITUAL_PLACES.map((place) => ({
    slug: place.slug,
  }));
}

export function generateMetadata({ params }: SpiritualPlacePageProps) {
  const place = getSpiritualPlaceBySlug(params.slug);

  return {
    title: place ? `${place.profession} - HI HILLS` : "Package Not Found - HI HILLS",
    description: place?.overview,
  };
}

export default function SpiritualPlacePage({ params }: SpiritualPlacePageProps) {
  const place = getSpiritualPlaceBySlug(params.slug);
          ...
          <div className="relative z-10">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur-sm">
                ...
                <p className="mt-2 text-2xl font-bold text-white">₹{place.price}/-</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur-sm">
                <p className="text-xs uppercase tracking-[0.2em] text-slate-300">Duration</p>
                <p className="mt-2 text-lg font-semibold text-white">{place.duration}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/10 p-4 backdrop-blur-sm">
                <p className="text-xs uppercase tracking-[0.2em] text-slate-300">Route</p>
                <p className="mt-2 text-sm font-semibold text-white">{place.route}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  A simple day-by-day view of your spiritual package.
                </p>
              </div>
            </div>
